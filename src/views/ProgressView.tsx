...
export function ProgressView() {
  ...
  const totalMinutes = completed.reduce((sum, item) => sum + actualStudyMinutes(item), 0)
  ...
  return <div className="page progress-page">
    <header className="page-header"><span className="eyebrow">나의 진도</span><h1>공부 시간을 한눈에 봅니다.</h1><p>시작·종료 시각으로 계산한 공부량과 완료 기록을 일간, 주간, 월간으로 확인하세요.</p></header>
    <section className="progress-highlights"><div><span className="metric-icon mint"><Activity /></span><small>전체 완료율</small><strong>{overallRate}%</strong><p>휴식일·취소 일정 제외</p></div><div><span className="metric-icon blue"><Clock3 /></span><small>누적 학습 시간</small><strong>{Math.floor(totalMinutes / 60)}<em>시간</em> {totalMinutes % 60}<em>분</em></strong><p>완료 일정의 실제 공부량</p></div><div><span className="metric-icon apricot"><TrendingUp /></span><small>연속 학습일</small><strong>{streak}<em>일</em></strong><p>휴식일은 흐름을 끊지 않아요</p></div></section>
    <section className="study-time-dashboard" aria-label="기간별 공부량 대시보드">
      ...
      <div className="study-period-summary"><div><span>계획 공부량</span><strong>{formatStudyMinutes(dashboard.planned)}</strong></div><div><span>완료 공부량</span><strong>{formatStudyMinutes(dashboard.actual)}</strong></div><div><span>시간 달성률</span><strong>{dashboard.percent}%</strong></div><div><span>완료 세션</span><strong>{dashboard.completedSessions}개</strong></div></div>
      <div className="period-bar-chart" style={{ gridTemplateColumns: `repeat(${dashboard.buckets.length}, minmax(34px, 1fr))` }} aria-label={`${periodLabels[period]} 공부 시간 막대 그래프`}>
        {dashboard.buckets.map((bucket, index) => <div key={`${bucket.label}-${index}`}><span className="bar-value">{bucket.actual ? formatStudyMinutes(bucket.actual) : ''}</span><div className="bar-track" title={`${bucket.label}: 계획 ${formatStudyMinutes(bucket.planned)}, 완료 ${formatStudyMinutes(bucket.actual)}`}><span className="planned-bar" style={{ height: `${Math.max(bucket.planned ? 4 : 0, (bucket.planned / maxBucketMinutes) * 100)}%` }} /><span className="actual-bar" style={{ height: `${Math.max(bucket.actual ? 4 : 0, (bucket.actual / maxBucketMinutes) * 100)}%` }} /></div><small>{bucket.label}</small></div>)}
      </div>
      <div className="chart-legend"><span><i className="planned" /> 계획</span><span><i className="actual" /> 완료</span></div>
    </section>
    ...
    <section className="gentle-note"><span>시간 기록 안내</span><p>시작 시각과 종료 시각을 입력하면 공부 시간이 자동 계산됩니다. 일정을 완료하면 해당 시간이 실제 공부량에 반영됩니다.</p></section>
  </div>
}
