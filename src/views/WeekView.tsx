...
export function WeekView() {
  ...
  const expectedMinutes = rangeItems.reduce((sum, item) => sum + item.estimatedMinutes, 0)
  const actualMinutes = rangeItems.reduce((sum, item) => sum + (item.actualMinutes ?? 0), 0)
  ...
  return <div className="page week-page">
    ...
    <section className="week-summary-line"><div><span>주간 완료율</span><strong>{rangeItems.length ? Math.round((completed / rangeItems.length) * 100) : 0}%</strong></div><div><span>예상 공부</span><strong>{Math.floor(expectedMinutes / 60)}시간 {expectedMinutes % 60}분</strong></div><div><span>실제 공부</span><strong>{Math.floor(actualMinutes / 60)}시간 {actualMinutes % 60}분</strong></div><div><span>학습 항목</span><strong>{rangeItems.length}개</strong></div></section>
    <section className="paper-week" aria-label="주간 일정">
      {days.map((day) => {
        ...
        return <article key={date} className={`${rest ? 'rest-day' : ''} ${isToday ? 'today' : ''}`}>
          ...
          <footer><Clock3 size={13} /> {dayItems.reduce((sum, item) => sum + item.estimatedMinutes, 0)}분</footer>
        </article>
      })}
    </section>
