...
export function ScheduleRow({ item, compact = false }: { item: ScheduleItem; compact?: boolean }) {
  ...
  const applyEdit = () => {
    editScheduleOccurrence(item.id, {
      ...
      estimatedMinutes: Math.max(0, editMinutes),
  ...
  return <div className={`schedule-row ${completed ? 'is-complete' : ''} ${compact ? 'compact' : ''}`}>
    ...
    <button className="schedule-main" onClick={() => setExpanded((value) => !value)} aria-expanded={expanded}>
      ...
      <span className="schedule-copy"><strong>{item.title}</strong><small>{category?.name ?? '직접 추가'} · {item.startTime || '시간 미정'}{item.estimatedMinutes ? ` · ${item.estimatedMinutes}분` : ''}</small></span>
    ...
    {expanded && <div className="schedule-detail">
      <div className="detail-stats"><span><Clock3 size={15} /> 계획 {item.plannedSequence ? `${item.plannedSequence}회차` : `${item.estimatedMinutes}분`}</span>{item.actualSequence && <span>실제 완료 {item.actualSequence}회차</span>}{item.actualMinutes && <span>실제 {item.actualMinutes}분</span>}</div>
      <div className="schedule-edit-grid">
        ...
        <label className="field"><span>시작 시각</span><input type="time" value={editTime} onChange={(event) => setEditTime(event.target.value)} /></label>
        <label className="field"><span>예상 시간</span><input type="number" min={0} step={5} value={editMinutes} onChange={(event) => setEditMinutes(Number(event.target.value))} /></label>
      ...
      <div className="schedule-actions">
        <button className="apply-schedule-edit" onClick={applyEdit}>변경 적용</button>
