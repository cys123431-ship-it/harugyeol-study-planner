...
export function AddItemDialog({ onClose, initialDate }: { onClose: () => void; initialDate?: string }) {
  ...
  const submit = (event: FormEvent) => {
    ...
    addManualItem({ title: title.trim(), date, startTime: time || undefined, estimatedMinutes: minutes, notes, priority, categoryId: data?.categories[0]?.id })
    onClose()
  }
    ...
    <section className="dialog compact-dialog" role="dialog" aria-modal="true" aria-labelledby="item-dialog-title">
      ...
      <form onSubmit={submit}>
        ...
        <div className="form-grid">
          ...
          <label className="field"><span>시작 시간 <small>선택</small></span><input type="time" value={time} onChange={(event) => setTime(event.target.value)} /></label>
          <label className="field"><span>예상 시간</span><select value={minutes} onChange={(event) => setMinutes(Number(event.target.value))}><option value={15}>15분</option><option value={30}>30분</option><option value={50}>50분</option><option value={60}>1시간</option><option value={90}>1시간 30분</option></select></label>
        ...
        <footer><button type="button" className="text-button" onClick={onClose}>취소</button><button className="primary-button" disabled={!title.trim()}>일정 추가</button></footer>
      </form>
    </section>
  </div>
}
